...
void Graphics2D::Init() {
	...
	programYUYV = esLoadProgram(vertShaderYUYV420, fragShaderYUYV420);
	//programYUYV = esLoadProgram(vertShaderYUYV420, fragShaderYUYV_Gray);

	programGray16 = esLoadProgram(vertShaderSrc, fragShaderGray16);
	...
	u_width = glGetUniformLocation(programYUYV, "u_width");
	u_coef = glGetUniformLocation(programGray16, "u_coef");
...
void Graphics2D::drawImageBGR565(cv::Mat& im) {
	// Set the viewport
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, im.cols, im.rows, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, im.data);
...
void Graphics2D::drawImageYUYV(cv::Mat& im) {
	// Set the viewport
	glViewport(0, 0, esContext.width, esContext.height);
	// Use the program object
	glUseProgram(programYUYV);

	for(int i = 0; i < NB_TEXTURE; i++) {
		glActiveTexture(GL_TEXTURE0 + i);
		glBindTexture(GL_TEXTURE_2D, texture[i]);
	}
	glUniform1f(u_width, (float)im.cols);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, im.cols/2, im.rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, im.data);
...
void Graphics2D::drawImageGray8(cv::Mat& im) {
	// Set the viewport
	glViewport(0, 0, esContext.width, esContext.height);
	// Use the program object
	glUseProgram(programID);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, im.cols, im.rows, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, im.data);
...
void Graphics2D::drawImageGray16(cv::Mat& im, GLfloat coef) {
	// Set the viewport
	glViewport(0, 0, esContext.width, esContext.height);
	// Use the program object
	glUseProgram(programGray16);

	for(int i = 0; i < NB_TEXTURE; i++) {
		glActiveTexture(GL_TEXTURE0 + i);
		glBindTexture(GL_TEXTURE_2D, texture[i]);
	}
	glUniform1f(u_coef, coef);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, im.cols, im.rows, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, im.data);
